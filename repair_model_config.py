...
# Import all necessary models and data classes
from src.data.dataloader import create_dataloaders
from src.models.multitask_model import MultitaskModel
from src.models.baseline_git import GitBaselineModel
# --- FIX: Import your custom DirectVLM class to use as the model shell ---
...
def parse_json_output(json_string: str) -> str:
    """
    Safely parses a JSON string and extracts title and description.
    """
    try:
...
class QualitativeExampleGenerator:
    ...
    def generate_side_by_side_examples(self, mtl_model_path, 
                                     ...
                                     t5_vlm_config_path, t5_vlm_model_path, # t5_vlm_config is now used
        ...
        # --- START: ROBUST LOADING FOR ViT-T5 MODEL ---
        print("Manually loading fine-tuned ViT-T5 VLM...")
        with open(t5_vlm_config_path, 'r') as f:
            t5_vlm_config = yaml.safe_load(f)

        # 1. Create an empty shell of your model with the correct architecture.
        ...
        # 2. Load the saved weights from your .bin file into the shell.
        weights_path = os.path.join(t5_vlm_model_path, 'pytorch_model.bin')
        print(f"Loading weights from: {weights_path}")
        t5_vlm_model.load_state_dict(torch.load(weights_path, map_location=self.device))
        t5_vlm_model.eval().to(self.device)

        # 3. Load the tokenizer from the same directory.
        t5_vlm_tokenizer = AutoTokenizer.from_pretrained(t5_vlm_model_path)
        # --- END: ROBUST LOADING ---
        ...
        with torch.no_grad():
            for batch in tqdm(self.val_loader, desc="Generating examples"):
                ...
                # --- 2. Get Predictions ---
                mtl_outputs = mtl_model.predict(pixel_values, self.tokenizer, self.mappings, use_hierarchical_prompt=True, no_repeat_ngram_size=2)[0]
                git_vlm_outputs = git_vlm_model.predict(pixel_values, git_processor, no_repeat_ngram_size=2)[0]
                
                # The generate method now comes from our custom DirectVLM class
                generated_jsons = t5_vlm_model.generate(pixel_values, t5_vlm_tokenizer)
                parsed_t5_vlm_text = parse_json_output(generated_jsons[0])

                true_text = self.tokenizer.batch_decode(batch['labels'], skip_special_tokens=True)[0]
                
                example = {
                    'index': len(examples), 'image_tensor': pixel_values[0].cpu(),
                    'ground_truth': true_text, 'mtl_hierarchical': mtl_outputs['generated_text'],
                    'mtl_attributes': mtl_outputs['predicted_attributes'], 'mtl_price': float(mtl_outputs['predicted_price']),
                    'vlm_git_base': git_vlm_outputs, 'vlm_vit_t5': parsed_t5_vlm_text,
                    'vlm_vit_t5_raw_json': generated_jsons[0]
                }
    ...
    def create_comparison_figures(self, examples, output_dir):
        ...
        for i, example in enumerate(tqdm(examples, desc="Creating figures")):
            ...
            for j, (title, text) in enumerate(texts):
                wrapped_text = textwrap.fill(str(text), width=45)
                axes[j+1].text(0.05, 0.95, f"{title}:\n\n{wrapped_text}", 
                             transform=axes[j+1].transAxes, verticalalignment='top', fontsize=9)
                axes[j+1].axis('off')
...
def main():
    ...
    parser.add_argument('--mtl_model', type=str, required=True)
    parser.add_argument('--git_vlm_config', type=str, required=True)
    parser.add_argument('--git_vlm_model', type=str, required=True)
    # --- FIX: Added the config file argument for the ViT-T5 model ---
    parser.add_argument('--t5_vlm_config', type=str, required=True, help='Path to the ViT-T5 VLM config file (e.g., configs/config_direct_vlm.yaml).')
    parser.add_argument('--t5_vlm_model', type=str, required=True, help='Path to the trained ViT-T5 VLM checkpoint DIRECTORY.')
    ...
    generator.generate_side_by_side_examples(
        ...
        t5_vlm_config_path=args.t5_vlm_config, # Pass the new argument
